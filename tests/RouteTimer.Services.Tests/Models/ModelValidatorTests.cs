...
public sealed class ModelValidatorTests
{
    ...
    private static CleanedActivity Activity(double movingSeconds)
    {
        ...
        return new CleanedActivity("Ride", samples, TimeSpan.FromSeconds(movingSeconds), new ActivityQuality(ActivityEligibility.Eligible, 1, 1, 1, 1, new Dictionary<string, int>(), []), ActivityFixtures.Metadata("ride.fit", start, start.AddSeconds(movingSeconds)));
    }
    ...
    private static CleanedActivity IneligibleActivity()
    {
        ...
        return new CleanedActivity("Short Ride", samples, TimeSpan.FromMinutes(1), new ActivityQuality(ActivityEligibility.Ineligible, 0.1, 0.1, 0.1, 0, new Dictionary<string, int> { ["gap"] = 1 }, ["too-short"]), ActivityFixtures.Metadata("short-ride.fit", start, start));
    }
